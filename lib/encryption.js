...
function generateKey(magicKey, id_key_uuid, privatePassword) {
  const password = privatePassword.length <= 8 ? privatePassword : Buffer.from(privatePassword).toString('base64');
  return Buffer.concat([
    MD5( Buffer.concat([ Buffer.from(id_key_uuid, 'utf8'), magicKey ]) ),
    MD5( Buffer.concat([ magicKey, Buffer.from(password, 'utf8') ]) )
  ]);
}
