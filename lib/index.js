// HACK! Work around an issue in node-xmpp-client:
// https://github.com/node-xmpp/client/issues/206
if (process.version.indexOf('v6') === 0) {
  const crypto      = require('crypto');
  var createHash    = crypto.createHash.bind(crypto);
  crypto.createHash = function(algo) {
    var hash = createHash.apply(crypto, arguments);
    if (algo === 'md5') {
      var update  = hash.update;
      hash.update = function(s) {
        if (arguments.length !== 2) {
          return update.call(hash, s, 'binary');
        }
        return update.apply(hash, arguments);
      }
    }
    return hash;
  };
